...
if [ -d "$WORK_DIR/.git" ]; then
  ...
  info "Cloning repo..."
  git clone https://github.com/mrfunntastiic/hash256-mine "$WORK_DIR"
  cd "$WORK_DIR"
fi
...
echo ""
echo "  4. Atau jalankan di background (tetap jalan walau SSH disconnect)"
echo "     nohup npm run start:gpu > miner.log 2>&1 &"
